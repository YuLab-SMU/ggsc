...
Here we use a example data from a single sample (sample 151673) of human brain dorsolateral prefrontal cortex (DLPFC) in the human brain, measured using the 10x Genomics Visium platform. First, a brief/standard data pre-processing were done with the `scater` and `scran` packages.
...
This data contain the data that are not in tissue, first we remove the cells not in tissue by the following codes. Then Cell-specific biases are normalized using the `computeSumFactors` method.
...
Next, we use the Graph-based clustering method to do the reduction with the `runPCA` and `runTSNE` functions of `scater` package.
...
Here, we used the `sc_dim` of `ggsc` package to visualize the result if `TSNE`, unlike other packages to visualize the results, `ggsc` also provides `sc_dim_geom_label` to visualize the cell cluster labels, which can work with `geom_shadowtext` of `shadowtext` and `geom_text` of `ggplot2` (default) by specifying the `geom` argument.
...
To visualize the gene expression of cells in the result of reduction, `ggsc` also provides `sc_feature` function to highlight on a dimensional reduction plot.
...
It also provides `sc_dim_geom_ellipse` to add the ellipse based the cluster result, and `sc_dim_geom_sub` to select and highlight a specific cluster cells. 
